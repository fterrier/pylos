...
(defn join-game [app control-ch game-id]
  (om/transact! app #(merge % initial-state))
  (om/update! app :game-id game-id)
  (put! control-ch {:action :server/join-game :game-id game-id}))

(defn- play-move [board move control-ch]
  (put! control-ch {:action :server/play-move :game-infos {:board board :size (board-size board) :move move}}))

(defn play-current-move [app control-ch]
  (let [current-game-infos (current-game-infos (:current-index app))
        current-selections (:selections (:current-move app))
...
(defn select-current-position [app control-ch current-position]
  (let [current-index      (:current-index app)
        current-selections (:selections (:current-move app))
        current-game-infos (current-game-infos (:current-index app))]
